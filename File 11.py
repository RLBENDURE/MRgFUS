...
#
#  CEM43 Reference (PMC):
#  - 17 CEM43:   ExAblate lesion boundary predictor
#                (0.37 ± 0.57 mm border accuracy)
#  - 36 CEM43:   Robust predictor of 1-day T1 lesion volume
#  - 13.6 CEM43: 50% tissue damage probability threshold
#  - Lesion size: 5.3 ± 1.2 mm (T1) to 6.2 ± 1.3 mm (T2)
#  - Focus width: ~1 mm
#
#  Thermal Protocol Reference:
#  - Cooling medium: degassed water at 15°C
#  - Scalp maintained <19°C
#  - Post high-energy (≥40 kJ) cooling: ~20 min
...
# ── CEM43 clinical thresholds ──────────────────────────────────────────────────
CEM43_DAMAGE_50PCT    = 13.6   # 50% tissue damage probability
CEM43_BOUNDARY_PRED   = 17.0   # ExAblate lesion boundary predictor
CEM43_T1_LESION_PRED  = 36.0   # Robust T1 lesion volume predictor (1-day post-op)
LESION_T1_MEAN_MM     = 5.3    # Mean T1 lesion diameter (mm)
LESION_T1_STD_MM      = 1.2
LESION_T2_MEAN_MM     = 6.2    # Mean T2 lesion diameter (mm)
LESION_T2_STD_MM      = 1.3
LESION_BORDER_ERR_MM  = 0.37   # Mean border estimation error (mm)
LESION_BORDER_STD_MM  = 0.57
FOCUS_WIDTH_MM        = 1.0    # Approximate ultrasound focus width
...
# ── 2. CEM43 clinical interpreter ─────────────────────────────────────────────
def interpret_cem43(cumulative_cem43: float) -> dict:
    """
    Classifies cumulative CEM43 against all clinical thresholds and
    estimates lesion size based on thermal dose.
    """
    # Lesion size estimation: scales linearly from 50% damage threshold
    # to T1 predicted size at 36 CEM43
    if cumulative_cem43 < CEM43_DAMAGE_50PCT:
        ...
        lesion_note   = (
            ...
            "Insufficient thermal dose for reliable ablation."
        )
        ...
        lesion_color  = "#f39c12"
        lesion_note   = (
            f"Approaching ExAblate boundary threshold ({CEM43_BOUNDARY_PRED} CEM43).\n"
            "Lesion forming but boundary prediction not yet reliable."
        )
        ...
        est_lesion_mm = (
            LESION_T1_MEAN_MM + frac * (LESION_T2_MEAN_MM - LESION_T1_MEAN_MM)
        )
        lesion_tier   = "THERAPEUTIC"
        lesion_color  = "#2ecc71"
        lesion_note   = (
            f"Above ExAblate boundary predictor ({CEM43_BOUNDARY_PRED} CEM43).\n"
            f"Est. lesion: {est_lesion_mm:.1f} mm  |  "
            f"Border accuracy: {LESION_BORDER_ERR_MM} ± {LESION_BORDER_STD_MM} mm"
        )
    else:
        lesion_prob   = 95.0 + min(5.0, (cumulative_cem43 - CEM43_T1_LESION_PRED) * 0.1)
        ...
        lesion_color  = "#27ae60"
        lesion_note   = (
            f"Above T1 lesion predictor ({CEM43_T1_LESION_PRED} CEM43).\n"
            f"Est. lesion: {est_lesion_mm:.1f} mm  |  "
            f"1-day T1 volume reliably predicted."
        )

    # T2 estimated size (slightly larger)
...
def simulate_passes(
    ...
    for p in range(1, n_passes + 1):
        ...
        inter_pass_cooling = (
            SKULL_COOLING_TIME_S if cumulative_e >= HIGH_ENERGY_THRESHOLD
        ...
        ) * np.exp(-cool_t / (inter_pass_cooling * 0.30))
        ...
        full_len      = len(heat_temp) + len(hold_temp) + len(cool_temp)
        ...
        scalp_profile = np.clip(
            scalp_profile + np.random.normal(0, 0.2, full_len),
...
def interpret_passes(sim: dict) -> str:
    ...
    lines = [
        ...
        f"  Protocol:  {protocol} | "
        f"Cooling: {COOLING_WATER_TEMP_C}°C degassed water",
        "-" * 75,
        "  CEM43 THRESHOLDS (PMC Reference)",
        f"  13.6 CEM43 → 50% tissue damage probability   "
        f"{'✔ MET' if cem43_info['damage_50pct_met'] else '✘ NOT MET'}",
        f"  17.0 CEM43 → ExAblate lesion boundary pred.  "
        f"{'✔ MET' if cem43_info['boundary_pred_met'] else '✘ NOT MET'}",
        f"  36.0 CEM43 → 1-day T1 lesion volume pred.    "
        f"{'✔ MET' if cem43_info['t1_pred_met'] else '✘ NOT MET'}",
        f"  Final CEM43: {sim['total_cem43']:.4f}  |  "
        f"Tier: {cem43_info['lesion_tier']}  |  "
        f"Damage prob: {cem43_info['lesion_prob_pct']}%",
        f"  Est. Lesion (T1): {cem43_info['est_lesion_t1_mm']:.2f} mm  |  "
        f"Est. Lesion (T2): {cem43_info['est_lesion_t2_mm']:.2f} mm",
        f"  Border accuracy: {LESION_BORDER_ERR_MM} ± {LESION_BORDER_STD_MM} mm  |  "
        f"Focus width: ~{FOCUS_WIDTH_MM} mm",
        "-" * 75,
        f"  Model runtime: {comp_time_ms:.2f} ms  |  "
        f"Surgeon planning: {PLANNING_TIME_LOW_MIN}–{PLANNING_TIME_HIGH_MIN} min  |  "
        f"Procedure: {TOTAL_PROC_LOW_HRS:.0f}–{TOTAL_PROC_HIGH_HRS:.0f} hrs",
        "-" * 75,
        "  ENERGY DELIVERY PLAN",
        f"  {'#':<4} {'Type':<14} {'Plan (J)':<11} {'Target°C':<10} "
        f"{'CEM43':<9} {'Lesion Tier':<15} {'Notes'}",
        "-" * 75,
    ]
    ...
    for p in passes:
        ...
        if p["pass_type"] == "Confirmation":
            notes = "Low power — targeting verification"
        elif not p["ablation_achieved"]:
            notes = "Escalate power if target temp not reached"
        else:
            notes = "Therapeutic — maintain hold duration"
        lines.append(
            f"  {p['pass_number']:<4} "
            f"{p['pass_type']:<14} "
...
def plot_per_pass(sim: dict, save_path: str = None):
    ...
    for i, (field, value) in enumerate(profile_rows):
        ...
        if field == "Damage Prob.":
            prob_val = final_ci["lesion_prob_pct"]
            bg = (
                "#d5f5e3" if prob_val >= 80
                else "#fef9e7" if prob_val >= 50
                else "#fadbd8"
            )

        ax_profile.add_patch(mpatches.FancyBboxPatch(
            (0.0, y - 0.022), 1.0, row_h,
        ...
        val_color = (
            ...
            else lesion_color if field in ("CEM43 Tier", "Est. Lesion T1",
                                           "Est. Lesion T2", "Damage Prob.")
            else "#222222"
        )
        ax_profile.text(0.96, y + 0.001, value,
                        ha="right", va="center",
                        fontsize=7.8, fontweight="bold", color=val_color)

    # Timing + computation footer
    timing_y = y_start - len(profile_rows) * row_h - 0.005
    ax_profile.add_patch(mpatches.FancyBboxPatch(
        (0.0, timing_y - 0.10), 1.0, 0.11,
        boxstyle="round,pad=0.02",
    ...
    ax_profile.text(0.5, timing_y - 0.020,
                    f"Model runtime:  {comp_time_ms:.2f} ms",
                    ha="center", va="center",
                    fontsize=7.8, color="#16a085", fontweight="bold")
    ax_profile.text(0.5, timing_y - 0.040,
                    f"Surgeon planning (NIH):  "
                    f"{PLANNING_TIME_LOW_MIN}–{PLANNING_TIME_HIGH_MIN} min",
                    ha="center", va="center",
                    fontsize=7.5, color="#555555")
    ax_profile.text(0.5, timing_y - 0.058,
                    f"Total prep (NIH):  "
                    f"{TOTAL_PREP_LOW_MIN}–{TOTAL_PREP_HIGH_MIN} min",
                    ha="center", va="center",
                    fontsize=7.5, color="#555555")
    ax_profile.text(0.5, timing_y - 0.076,
                    f"Total procedure:  "
    ...
    ax_hist.axvline(mu, color=kt_color, linewidth=1.0, linestyle="--",
                    alpha=0.6, label=f"Mean SDR {mu:.2f}")
    ...
    # ══ CEM43 Clinical Thresholds Panel ═══════════════════════════════════════
    ax_cem43pan.set_xlim(0, 1)
    ax_cem43pan.set_ylim(0, 1)
    ax_cem43pan.axis("off")
    ax_cem43pan.set_title("CEM43 Clinical Thresholds", fontsize=11, fontweight="bold")
    ...
    for (thresh, label, met, tcolor), ty in zip(thresholds, thresh_y):
        ...
        fill_w     = min(1.0, sim["total_cem43"] / thresh) if thresh > 0 else 0
        fill_w     = min(fill_w, 0.98)
    ...
    ax_cem43pan.add_patch(mpatches.FancyBboxPatch(
        (0.02, 0.09), 0.96, 0.22,
        boxstyle="round,pad=0.02",
        linewidth=1.5, edgecolor=lesion_color, facecolor=lesion_color + "18"
    ))
    ax_cem43pan.text(0.5, 0.29,
                     f"Estimated Lesion Size",
                     ha="center", va="center",
    ...
    ax_cem43pan.text(0.5, 0.102,
                     ...
                     f"Border err: {LESION_BORDER_ERR_MM} ± "
                     f"{LESION_BORDER_STD_MM} mm",
    ...
    ax_cem.axhline(CEM43_BOUNDARY_PRED, color="#8e44ad", linewidth=1.1,
                   linestyle="--",
                   label=f"Boundary pred. ({CEM43_BOUNDARY_PRED})")
    ax_cem.axhline(CEM43_T1_LESION_PRED, color="#2980b9", linewidth=1.0,
                   linestyle="-.",
                   label=f"T1 lesion pred. ({CEM43_T1_LESION_PRED})")
    ...
    ax_lesion.plot(pass_nums, pass_lesion_t1, color="#c0392b",
                   ...
                   label=f"Est. T1 lesion (mm)  ref: {LESION_T1_MEAN_MM}±{LESION_T1_STD_MM}")
    ax_lesion.plot(pass_nums, pass_lesion_t2, color="#922b21",
                   linewidth=1.6, marker="s", markersize=5,
                   linestyle="--",
                   label=f"Est. T2 lesion (mm)  ref: {LESION_T2_MEAN_MM}±{LESION_T2_STD_MM}")
    ...
    ax_lesion.axhline(FOCUS_WIDTH_MM, color="#555555", linewidth=0.9,
                      linestyle=":", label=f"Focus width (~{FOCUS_WIDTH_MM} mm)")

    ax2_lesion = ax_lesion.twinx()
    ax2_lesion.plot(pass_nums, pass_dmg_prob, color="#8e44ad",
                    linewidth=1.5, marker="^", markersize=5,
                    linestyle="-.", zorder=3, label="Damage prob. (%)")
    ax2_lesion.axhline(50, color="#8e44ad", linewidth=0.8,
                       linestyle=":", alpha=0.7)
    ...
    lines1, labels1 = ax_lesion.get_legend_handles_labels()
    lines2, labels2 = ax2_lesion.get_legend_handles_labels()
    ax_lesion.legend(lines1 + lines2, labels1 + labels2,
                     fontsize=7.0, loc="upper left", framealpha=0.85)
    ax_lesion.set_xlabel("Pass Number", fontsize=10)
    ax_lesion.set_ylabel("Estimated Lesion Size (mm)", fontsize=10)
    ax_lesion.set_title(
        "Estimated Lesion Size & Damage Probability per Pass\n"
        f"(Border accuracy: {LESION_BORDER_ERR_MM} ± {LESION_BORDER_STD_MM} mm)",
        fontsize=11, fontweight="bold"
    )
    ...
    fig.text(
        ...
        ha="center", va="center",
        fontsize=9.0, color="#2c3e50", fontweight="bold",
        bbox=dict(
